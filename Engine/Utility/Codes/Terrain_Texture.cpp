...
, m_ih(rhs.m_ih)
{
	...
	memcpy(m_pPosition,rhs.m_pPosition,sizeof(_vec3)*m_dwVtxCnt);
}
...
HRESULT Engine::CTerrain_Texture::Ready_Buffer(const _ulong& dwCntX, const _ulong& dwCntZ, const _ulong& dwVtxInterval)
{
	...
	// 이미지로부터 정점의 높이를 얻어온다.
	m_hFile = CreateFile(L"../Bin/Resources/Texture/Terrain/Height.bmp", GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
	...
	for	(_ulong i = 0; i < dwCntZ; ++i)
	{
		for (_ulong j = 0; j < dwCntX; ++j)
		{
			...
			pVertex[dwIndex].vPos = _vec3((_float)j * dwVtxInterval, (pPixel[dwIndex] & 0x000000ff) / 20.0f/*0.0f*/, (_float)i * dwVtxInterval);
			m_pPosition[dwIndex] = pVertex[dwIndex].vPos;
			pVertex[dwIndex].vNormal = _vec3(0.f, 0.f, 0.f);
			pVertex[dwIndex].vTexUV = _vec2(_float(j) / (dwCntX - 1) * 30.f, _float(i) / (dwCntZ - 1)*30.f);
		}
	}
